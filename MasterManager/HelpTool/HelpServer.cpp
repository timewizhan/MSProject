...
#include "..\Server\MMServerError.h"
...
DWORD CHelpServer::InitWSAServerSock(ST_SERVER_CONTEXT &refstServerContext)
{
	DWORD dwRet;
	dwRet = ::WSAStartup(MAKEWORD(2, 2), &refstServerContext.stWSAData);
	if (dwRet != 0) {
	...
	refstServerContext.stServerInfo.hServerSock = ::WSASocket(AF_INET, SOCK_STREAM, IPPROTO_TCP, NULL, 0, WSA_FLAG_OVERLAPPED);
	if (refstServerContext.stServerInfo.hServerSock == INVALID_SOCKET) {
		dwRet = WSAGetLastError();
		ShowErrorWSASocket(dwRet);
		return E_RET_FAIL;
...
DWORD CHelpServer::AcceptServer(ST_SERVER_CONTEXT &refstServerContext, ST_CLIENT_SOCKET &refstClientSock)
{
	...
	refstClientSock.ClientAddress = szClientAddress;
